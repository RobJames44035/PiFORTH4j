...
class Roll extends AbstractRuntime {
 ...
 */
    ...
    Object execute(ForthInterpreter interpreter, Word word, Word parentWord) {
        Integer depth = interpreter.dataStack.pop() as Integer

        if (interpreter.dataStack.size() < depth + 1 || depth < 0) {
            throw new ForthInterpreterException("ROLL depth is out of bounds")
        }

        Integer rolledValue = interpreter.dataStack.get(interpreter.dataStack.size() - depth - 1)
        interpreter.dataStack.remove(interpreter.dataStack.size() - depth - 1)
        interpreter.dataStack.push(rolledValue)
        return null
    }
}
