...
class Until extends AbstractRuntime {
    ...
    Object execute(ForthInterpreter interpreter, Word word, Word parentWord) {
        ...
        if (condition == 0) { // if the condition is false, jump back to "BEGIN"
            Integer jumpToIndex = interpreter.returnStack.pop() as Integer
            parentWord.executionIndex = jumpToIndex
            interpreter.returnStack.push(jumpToIndex)
            interpreter.dataStack.push(condition)
        }
        return false
    }
}
