...
class Immediate extends AbstractRuntime {
    ...
    Object execute(ForthInterpreter interpreter, Word word, Word parentWord) {
        ...
        Word w = interpreter.wordService.findByName(token)
        if (w != null) {
            w.immediate = true
            interpreter.wordService.save(w)
        } else {
            throw new ForthInterpreterException("${token} not found.")
        }
        return null
    }
}
