...
class DNegate extends AbstractRuntime {
    ...
    Object execute(ForthInterpreter interpreter, Word word, Word parentWord) {
        int[] n = interpreter.dataStack.popDouble() as int[]
        int[] result = DoubleUtil.negateDouble(n)
        interpreter.dataStack.pushDouble(result[0], result[1])
        return null
    }
}
