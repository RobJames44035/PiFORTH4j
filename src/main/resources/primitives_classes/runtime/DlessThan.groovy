...
import com.rajames.forth.util.DoubleUtil

class DlessThan extends AbstractRuntime {
    ...
    Object execute(ForthInterpreter interpreter, Word word, Word parentWord) {
        int[] n1 = interpreter.dataStack.popDouble() as int[]
        int[] n2 = interpreter.dataStack.popDouble() as int[]
        boolean result = DoubleUtil.lessThanDouble(n1, n2)
        int forthBool = result ? -1 : 0
        interpreter.dataStack.push(forthBool)
