...
class While extends AbstractRuntime {
...
: test begin count @ 10 < while cr ." testing " count @ . count @ 1+ count ! repeat ;
    */
    ...
    Object execute(ForthInterpreter interpreter, Word word, Word parentWord) {
        // Find the index of 'REPEAT' command in parent word's forthWords list
        Integer repeatIndex = parentWord.forthWords.indexOf("repeat")

        parentWord.executionIndex = repeatIndex - 1 // index for 'REPEAT'. Compiler will increment.
        Integer condition = interpreter.dataStack.pop() as Integer
        parentWord.returned = condition != 0
        return null
    }
}
